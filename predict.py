...
#         creation_dt = to_ist(creation_dt)
#     first_msg_dt = min([m["dt"] for m in msgs if m["dt"]], default=None)
#     start_anchor = min([d for d in [creation_dt, first_msg_dt] if d], default=None)
...
#     if not mapped:
#         return {"error": "No users found in input"}
...
# if __name__ == "__main__":
#     ap = argparse.ArgumentParser()
#     ap.add_argument("--file", "-f", help="testing JSON file")
#     ap.add_argument("--model", "-m", default=MODEL_PATH)
#     ap.add_argument("--topk", "-k", type=int, default=5)
#     ap.add_argument("--rule-threshold", "-r", type=int, default=6)
...
#                               rule_threshold=args.rule_threshold, alpha=args.alpha)
